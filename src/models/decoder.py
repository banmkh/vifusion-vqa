...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, decoder_mask: torch.Tensor):
        residual = y
...
class Decoder(nn.Module):
    def __init__(self, d_model: int, ffn_hidden: int, num_heads: int, drop_prob: float, num_layers: int = 1):
        super().__init__()
